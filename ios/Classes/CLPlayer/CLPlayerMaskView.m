...
- (void)makeConstraints{
    ...
    [self.topToolBar mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(64+statusbarHeight);
    }];
    //底部工具条
    ...
    [self.backButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.topToolBar).mas_offset(10);
...
//    NSString *path = [[NSBundle mainBundle] pathForResource:@"movieIMG" ofType:@"bundle"];
//    NSBundle *bundle = [NSBundle bundleWithPath:path];
//    NSString *file = [bundle pathForResource:name ofType:@"png"];
//    UIImage *img = [[UIImage imageWithContentsOfFile:file]imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    UIImage *img = [[UIImage imageNamed:name] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    return img;
}
-(void)setIsFullScreens:(BOOL)isFullScreens{
    
    if (isFullScreens == YES) {
